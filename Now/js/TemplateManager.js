const TemplateManager = {
      ...
      while (el) {
        if (el.attributes && el.attributes.length > 0) {
          Array.from(el.attributes).forEach(attr => {
            ...
            const originalState = this.deepClone(context.state);
            ...
            const updateAttr = () => {
              try {
                const state = originalState || context.state;
                const ctx = originalContext || context;
                const evalScope = {...state, ...(ctx.computed || {}), ...(ctx.methods || {})};
            ...
            // Hook into reactive updates if available
  ...
  processDataText(el, expression, context) {
    ...
    try {
      if (!el._textBinding) {
        ...
        // Support attribute-based formatters similar to TableManager
        ...
        if (attrFormatters.length > 0) {
          el._textBinding.formatters = [
            ...el._textBinding.formatters,
            ...attrFormatters
          ];
        }
      } else {
        // Merge new state into existing to preserve data-for loop variables (e.g. lng, index)
        // while adding new data (e.g. keywords from setFormData)
        el._textBinding.originalState = {
          ...el._textBinding.originalState,
          ...this.deepClone(context.state)
        };
        el._textBinding.originalContext = {
          ...el._textBinding.originalContext,
          ...context
        };
      }
  ...
  processDataHtml(el, value, context) {
    ...
    try {
      if (!el._htmlBinding) {
        ...
        // Merge new state into existing to preserve data-for loop variables (e.g. lng, index)
        // while adding new data (e.g. keywords from setFormData)
        el._htmlBinding.originalState = {
          ...el._htmlBinding.originalState,
          ...this.deepClone(context.state)
        };
        el._htmlBinding.originalContext = {
          ...el._htmlBinding.originalContext,
          ...context
        };
      }
  ...
  processDataIf(el, expression, context) {
    ...
    try {
      if (!el._ifBinding) {
        ...
        // Merge new state into existing to preserve data-for loop variables (e.g. lng, index)
        // while adding new data (e.g. keywords from setFormData)
        el._ifBinding.originalState = {
          ...el._ifBinding.originalState,
          ...this.deepClone(context.state)
        };
        el._ifBinding.originalContext = {
          ...el._ifBinding.originalContext,
          ...context
        };
      }
  ...
  processDataClass(el, value, context) {
    ...
    try {
      if (!el._classBinding) {
        ...
        // Merge new state into existing to preserve data-for loop variables (e.g. lng, index)
        // while adding new data (e.g. keywords from setFormData)
        el._classBinding.originalState = {
          ...el._classBinding.originalState,
          ...this.deepClone(context.state)
        };
        el._classBinding.originalContext = {
          ...el._classBinding.originalContext,
          ...context
        };
      }
  ...
  processDataAttr(el, value, context) {
    ...
    try {
      if (!el._attrBinding) {
        ...
        // Merge new state into existing to preserve data-for loop variables (e.g. lng, index)
        // while adding new data (e.g. keywords from setFormData)
        el._attrBinding.originalState = {
          ...el._attrBinding.originalState,
          ...this.deepClone(context.state)
        };
        el._attrBinding.originalContext = {
          ...el._attrBinding.originalContext,
          ...context
        };
      }
      ...
      bindings.forEach(binding => {
        try {
          const updateAttr = () => {
            ...
            if (attrName === 'data') {
              this.processDataBind(el, expression, context);
              return;
            }

            // Use originalState from binding (captured at creation time with correct item context)
            const state = el._attrBinding.originalState || context.state;
            const ctx = el._attrBinding.originalContext || context;
            ...
            const evalScope = {...state, ...ctx.computed};
            let attrValue;
  ...
  processDataModel(el, value, context) {
    ...
    try {
      if (!el._modelBinding) {
        ...
        // focus/blur listeners are set up inside updateModel() with dedup guard

        // Parse modifiers from the END of the value string
        // Supported modifiers: .lazy, .number, .trim
        // E.g., "user.name.trim" → path="user.name", modifier trim=true
        // E.g., "user.name" → path="user.name", no modifiers
        const knownModifiers = Object.keys(el._modelBinding.modifiers);
        const parts = value.split('.');
        // Walk backward from the end collecting known modifiers
        while (parts.length > 1 && knownModifiers.includes(parts[parts.length - 1])) {
          const mod = parts.pop();
          el._modelBinding.modifiers[mod] = true;
        }
        el._modelBinding.value = parts.join('.');
      } else {
        // Update binding with new context when re-processing
  ...
  processDataChecked(el, expression, context) {
    ...
    try {
      if (!el._checkedBinding) {
        ...
        // Merge new state into existing to preserve data-for loop variables (e.g. lng, index)
        // while adding new data (e.g. tab from setFormData) — same pattern as processDataAttr.
        el._checkedBinding.originalState = {
          ...el._checkedBinding.originalState,
          ...this.deepClone(context.state)
        };
        el._checkedBinding.originalContext = {
          ...el._checkedBinding.originalContext,
          ...context
        };
      }
      ...
      const updateChecked = () => {
        ...
        try {
          ...
          // Use originalState so that data-for loop variables (e.g. lng, index)
          // captured at binding time are available during reactive re-evaluation.
          // This mirrors processDataAttr's approach.
          const state = el._checkedBinding.originalState || context.state;
          const ctx = el._checkedBinding.originalContext || context;

          // Merge live state on top so reactive state changes (e.g. tab) are picked up,
          // while preserving loop-local variables from originalState.
          const evalState = {...state, ...context.state, ...ctx.computed, ...context.computed};

          let isChecked = ExpressionEvaluator.evaluate(el._checkedBinding.expression, evalState, ctx);
  ...
  processDataOptionsKey(el, expression, context) {
    ...
    try {
      const optionsData = context.options;
      if (!optionsData || typeof optionsData !== 'object') return;

      const interpState = {...(context.state || {}), ...(context.computed || {})};
      let lookup = String(expression).trim();
      if (lookup.includes('{{')) {
        lookup = lookup.replace(/\{\{(.+?)\}\}/g, (_, inner) => {
          const val = ExpressionEvaluator.evaluate(inner.trim(), interpState, context);
          return val == null || val === undefined ? '' : String(val);
        }).trim();
      }
      if (!lookup) return;
      ...
      if (ExpressionEvaluator.bracketAccessRegex.test(lookup)) {
        options = ExpressionEvaluator.resolveBracketAccess(lookup, optionsData, context);
      } else if (lookup.includes('.')) {
        options = ExpressionEvaluator.getPropertyPath(lookup, optionsData, context);
      } else {
      ...
      const wrappedOptions = Object.assign({}, optionsData, {[INLINE_KEY]: options});
  ...
  processDataBind(el, expression, context) {
    ...
    try {
      ...
      const boundData = ExpressionEvaluator.evaluate(expression, {...state, ...(ctx.computed || {})}, ctx);
      ...
      if (isTable && window.TableManager) {
        ...
        const setTableData = () => {
          ...
          // Pass options to table.dataOptions if available
          const optionsData = context.options || state.options;
          if (optionsData && tableObj) {
            if (!tableObj.dataOptions) tableObj.dataOptions = {};
            Object.assign(tableObj.dataOptions, optionsData);
          }

          // Normalize bound data into TableManager canonical schema
  ...
  processDataFiles(el, expression, context) {
    ...
    try {
      if (!el._filesBinding) {
        ...
        el._filesBinding.originalState = this.deepClone(context.state);
        el._filesBinding.originalContext = {...context};
      }
      ...
      const updateFiles = () => {
        ...
        try {
          const state = el._filesBinding.originalState || context.state;
          const ctx = el._filesBinding.originalContext || context;

          const value = ExpressionEvaluator.evaluate(
            el._filesBinding.expression,
            {...state, ...ctx.computed},
            ctx
          );
