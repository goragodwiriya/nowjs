...
const ServiceWorkerManager = {
  ...
  async registerServiceWorker() {
    try {
      ...
      if (window.Now?.emit) {
        Now.emit('serviceworker:registered', {registration});
      }
      return registration;
    } catch (error) {
  ...
  async subscribePush() {
    ...
    try {
      ...
      if (window.Now?.emit) {
        Now.emit('serviceworker:push-subscribed', {subscription});
      }
      return subscription;
    } catch (error) {
  ...
  async unsubscribePush() {
    try {
      ...
      if (subscription) {
        ...
        if (window.Now?.emit) {
          Now.emit('serviceworker:push-unsubscribed');
        }
        return true;
      }
      return false;
  ...
  setupEventListeners(registration) {
    registration.addEventListener('updatefound', () => {
      ...
      if (window.Now?.emit) {
        Now.emit('serviceworker:update-found', {registration});
      }
    });
  ...
  handleStateChange(worker) {
    ...
    if (window.Now?.emit) {
      Now.emit('serviceworker:state-change', {state: worker.state, status: this.state.status});
    }
  },
  ...
  handleWorkerMessage(event) {
    ...
    switch (data.type) {
      ...
      case 'OFFLINE_READY':
        ...
        if (window.Now?.emit) Now.emit('serviceworker:offline-ready');
        break;
      case 'LOG':
    ...
    if (window.Now?.emit) Now.emit('serviceworker:message', data);
  },
  ...
  handleError(error, context = 'general') {
    ...
    if (window.Now?.emit) {
      Now.emit('serviceworker:error', errorInfo);
    }
  },
  ...
  async unregister() {
    ...
    try {
      ...
      if (success) {
        ...
        if (window.Now?.emit) {
          Now.emit('serviceworker:unregistered');
        }
      }
      return success;
    } catch (error) {
