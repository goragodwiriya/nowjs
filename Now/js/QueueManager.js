...
const QueueManager = {
  ...
  async init(options = {}) {
    try {
      ...
      if (window.Now?.emit) {
        Now.emit('queue:ready', {manager: 'QueueManager'});
      }
  ...
  createQueue(name, options = {}) {
    ...
    if (window.Now?.emit) {
      Now.emit('queue:created', {name, options: queue.options});
    }
  ...
  pauseQueue(queueName) {
    ...
    if (window.Now?.emit) {
      Now.emit('queue:paused', {queueName});
    }
  ...
  resumeQueue(queueName) {
    ...
    if (window.Now?.emit) {
      Now.emit('queue:resumed', {queueName});
    }
  ...
  clearQueue(queueName) {
    ...
    if (window.Now?.emit) {
      Now.emit('queue:cleared', {queueName, taskCount});
    }
  ...
  async loadPersistedTasks() {
    ...
    try {
      ...
      if (window.Now?.emit) {
        Now.emit('queue:tasks:loaded', {count: loadedCount});
      }
  ...
  pauseAll() {
    ...
    if (window.Now?.emit) {
      Now.emit('queue:all:paused', {count});
    }
  ...
  resumeAll() {
    ...
    if (window.Now?.emit) {
      Now.emit('queue:all:resumed', {count});
    }
  ...
  clearAll() {
    ...
    if (window.Now?.emit) {
      Now.emit('queue:all:cleared', {count});
    }
