...
const SecurityManager = {
  ...
  // ============ Input Sanitization (ไม่ validate) ============
  sanitizeInput(value) {
  ...
  setupHttpInterceptors() {
    ...
    window.http.addRequestInterceptor(async (config) => {
      ...
      // Sanitize request data (ไม่ validate)
  ...
  enhanceForm(form) {
    ...
    if (config.validation !== false) {
      this.applyFormValidation(form, config);
    }
  ...
  extractFormSecurityConfig(form) {
    return {
      ...
      validation: this.getDataBool(form, 'validation', this.config.validation.enabled),
  ...
  applyFormValidation(form, config) {
  ...
  validateForm(form) {
    ...
    for (const [name, value] of formData) {
      ...
      const error = this.validateFormField(field, value);
      if (error) {
        errors.push({field: name, error, element: field});
      }
    }
  ...
  validateFormField(field, value) {
    ...
    if (field.pattern && value) {
      ...
      if (!regex.test(value)) {
        return field.dataset.errorPattern || 'Please match the requested format';
      }
    }
  ...
  showFormErrors(form, errors) {
    ...
    // Show new errors
    errors.forEach(({field, error, element}) => {
      this.showFieldError(element, error);
    });

    // Show general error message
    const generalError = form.querySelector('.form-message, .login-message');
    if (generalError && errors.length > 0) {
      generalError.textContent = Now.translate('Please correct the errors below and try again.');
  ...
  showFieldError(field, message) {
    ...
    field.container?.classList.add(errorClass);

    // Add error class to element.wrapper (from ElementFactory)
    if (field.wrapper && field.wrapper !== field.container) {
      field.wrapper.classList.add(errorClass);
    }

    // Determine result element ID - check data-result attribute first, fallback to result_{field.name || field.id}
    const resultId = field.dataset?.result || `result_${field.name || field.id}`;
    let errorElement = document.getElementById(resultId);
    if (errorElement) {
      errorElement.textContent = Now.translate(message);
  ...
  clearFieldError(field) {
    ...
    field.container?.classList.remove(errorClass);

    // Remove error class from element.wrapper (from ElementFactory)
    if (field.wrapper && field.wrapper !== field.container) {
      field.wrapper.classList.remove(errorClass);
    }

    // Determine result element ID - check data-result attribute first, fallback to result_{field.name || field.id}
    const resultId = field.dataset?.result || `result_${field.name || field.id}`;
    const errorElement = document.getElementById(resultId);
    if (errorElement) {
  ...
  validateData(data, rules = {}) {
    ...
    for (const [field, value] of Object.entries(data)) {
      ...
      if (fieldRules) {
        const error = this.applyValidationRule(value, fieldRules);
        if (error) {
          errors[field] = error;
        }
      }
