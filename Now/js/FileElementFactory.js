class FileElementFactory extends ElementFactory {
  ...
  static parseExistingFiles(element) {
    try {
  ...
  static initSortable(container, config) {
    ...
    new Sortable(container, {
      ...
      onEnd: async (evt) => {
        ...
        try {
          const result = await simpleFetch.post(config.actionUrl, {
            action: 'sort',
            order: order
          });
  ...
  static async deleteFile(file, preview, config) {
    try {
      const confirmed = await DialogManager.confirm(Now.translate('Are you sure you want to delete this item?'));
      if (!confirmed) return;
      ...
      if (result.status === 200) {
        preview.remove();
        if (window.NotificationManager) {
          NotificationManager.success(Now.translate('Item deleted successfully'));
        }
