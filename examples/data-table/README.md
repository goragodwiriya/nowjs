...
```javascript
// เรนเดอร์เสร็จ ครอบคลุมทั้งโหลดใหม่ เรียงลำดับ กรอง และเปลี่ยนหน้า
EventManager.on('table:render', ({tableId, data, totalRecords, filteredRecords, isServerSide}) => {
});

// โหลดข้อมูลใหม่อัตโนมัติแต่ละรอบ
EventManager.on('table:refreshed', ({tableId, automatic, timestamp}) => {
});
...
EventManager.on('table:fieldChange', ({tableId, field, value, rowData, element}) => {
});

// ส่ง action ไป endpoint แล้วได้ผลกลับ
EventManager.on('table:action', ({tableId, action, success, response, timestamp}) => {
});

// เกิดข้อผิดพลาดระหว่างทำ action
EventManager.on('table:error', ({tableId, action, error, timestamp}) => {
});
```

รายชื่อ event ครบทั้ง 17 ตัวอยู่ใน [เอกสาร TableManager](../../docs/th/TableManager.md#เหตุการณ์)
