...
| `cascade:reset` | When the whole chain is reset |
| `cascade:error` | When an error occurs loading options |
| `cascade:destroy` | When the instance is destroyed |

CascadingSelectManager ส่ง event ผ่าน `EventManager` จึงต้องรับด้วย
`EventManager.on()` ไม่ใช่ `document.addEventListener()`

```javascript
EventManager.on('cascade:loaded', ({instance, element, options}) => {
  console.log('Options loaded for:', element.name);
